...
if(verifyToken(request.data.token , response)){
	...
	write(response, 400 , "Token Expired ");
}
});
...
let password = request.data.password
	...
	if(isUserExists){
	const token = sign({ username: username }, 'klsdjkldklsdkldslkjdldjkdsjkfj');
...
try{
const decoded = verify(token , "klsdjkldklsdkldslkjdldjkdsjkfj")

const time = (Date.now() / 1000 - decoded.iat) / 60
...
return true
	...
	write(response, 500, JSON.stringify({err}));
return false
}
